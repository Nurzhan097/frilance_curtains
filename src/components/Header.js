...
export default function Example() {
  const [open, setOpenHeader] = useState(false)

  return (
    <div className="absolute inset-x-0 top-0 z-50">
      {/* Mobile menu */}
      <Dialog open={open} onClose={setOpenHeader} className="relative z-40 lg:hidden">
        <DialogBackdrop
          transition
              ...
              <div className="border-b border-gray-200">
                <TabList className="-mb-px flex space-x-8 px-4">
                  {navigation.categories.map((category) => (
                    <Tab
                      key={category.name}
                      className="flex-1 border-b-2 border-transparent px-1 py-4 text-base font-medium whitespace-nowrap text-gray-900 data-selected:border-emerald-600 data-selected:text-emerald-600"
                    >
                      {category.name}
                    </Tab>
                  ))}
